...
import { Alert, AlertDescription, AlertIcon, Box, Button, Heading, Text, VStack } from "@chakra-ui/react";
...
export default function MethodRoute() {
  ...
  function handleTelegram() {
    ...
    window.Telegram.Login.auth(
      { bot_id: import.meta.env.VITE_TELEGRAM_BOT_USERNAME, request_access: true },
      async (data) => {
        if (!data) { setTgLoading(false); return; }
        try {
  ...
  return (
    <VStack spacing={5} align="stretch">
      ...
      >
        ...
        <Text color="text.muted" fontSize="xs" flexShrink={0}>Change</Text>
      </Box>
      ...
      <VStack spacing={3}>
        <Button variant="native" w="full" h={10} leftIcon={<MdLock size={18} />} onClick={() => { setMethod("password"); navigate("/register/setup"); }}>
          Password
        </Button>
        <Button variant="native" w="full" h={10} leftIcon={<MdVpnKey size={18} />} onClick={() => { setMethod("passkey"); navigate("/register/setup"); }}>
          Passkey
        </Button>
        <Button variant="google" w="full" h={10} leftIcon={<GoogleG />}>
          Continue with Google
        </Button>
        <Button variant="telegram" w="full" h={10} leftIcon={<FaTelegram size={20} />} isLoading={tgLoading} onClick={handleTelegram}>
          Continue with Telegram
        </Button>
      </VStack>
      {tgError && (
        <Alert status="error" borderRadius="sm" fontSize="xs">
          <AlertIcon /><AlertDescription>{tgError}</AlertDescription>
        </Alert>
      )}
    </VStack>
  );
